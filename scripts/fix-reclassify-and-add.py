#!/usr/bin/env python3
"""Reclassify and fix articles.ts — move articles to correct specialties and add new ones."""
import re

with open('src/data/articles.ts', 'r', encoding='utf-8') as f:
    t = f.read()

# Reclassification map (slug -> new specialty)
reclassify = {
    ...
    "doc-nhan-dinh-duong": "dinh-duong-tong-quat",
    "doc-nhan-thuc-pham": "dinh-duong-tong-quat",
}

# Apply reclassification using line-based editing
lines = t.split('\n')
current_slug = None
modified = 0
...
for i, line in enumerate(lines):
    ...
    if stripped.startswith('specialty: "dinh-duong-tong-quat"') and current_slug in reclassify:
        new_spec = reclassify[current_slug]
        indent = line[:len(line) - len(line.lstrip())]
        lines[i] = f'{indent}specialty: "{new_spec}",\n'
        modified += 1
    # Also handle already-reclassified specialties that should move
    if stripped.startswith('specialty: "') and current_slug in reclassify:
        current_spec = stripped.split('"')[1]
        target_spec = reclassify[current_slug]
        if current_spec != target_spec and current_spec != 'dinh-duong-tong-quat':
            indent = line[:len(line) - len(line.lstrip())]
            lines[i] = f'{indent}specialty: "{target_spec}",\n'
            modified += 1

t = '\n'.join(lines)
print(f"Reclassified {modified} articles")

# Now add new articles before the closing '];'
new_articles = [
    {
        ...
        "emoji": "🎀", "category": "disease", "dc": "dang-co-benh",
        "readTime": 8,
    ...
    },
    {
        ...
        "desc": "Xơ cứng bì hệ thống (scleroderma): khó nuốt, trào ngược, hội chứng kém hấp thu. Chế độ ăn mềm, chống viêm.",
        "emoji": "🫸", "category": "disease", "dc": "dang-co-benh",
        "readTime": 7,
    ...
    },
    {
        ...
        "emoji": "❤️", "category": "disease", "dc": "dang-co-benh",
        "readTime": 8,
...
entries_block = '\n'.join(make_entry(a) for a in new_articles)

# Insert before closing '];'
marker = '\n];\n\n// Index by slug'
insertion = entries_block + marker

if marker in t:
    t = t.replace(marker, insertion)
    with open('src/data/articles.ts', 'w', encoding='utf-8') as f:
        f.write(t)
    print(f"✅ Added {len(new_articles)} new articles to registry")
else:
