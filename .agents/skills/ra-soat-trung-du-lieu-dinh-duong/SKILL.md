...
1. Chạy `scripts/scan_duplicates.mjs` từ thư mục gốc dự án để lập danh sách va chạm slug/tên/alias trên catalog đang xuất bản; bộ quét bỏ qua food slug đã bị `duplicateFoodSlugsExcludedFromLibrary` loại khỏi thư viện và phân biệt food/recipe trong `nutrition.ts`.
...
6. Khi có thay đổi dữ liệu, chạy `npm run build`, kiểm tra slug trong `dist/api-foods.json` hoặc `dist/api-recipes.json` tùy loại dữ liệu, xác nhận `public/api/search-index.json` đã được cập nhật, ghi báo cáo trong `reports/`, rồi mới commit/push nếu người dùng đã yêu cầu.
