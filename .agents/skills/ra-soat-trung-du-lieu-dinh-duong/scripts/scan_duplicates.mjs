...
for (const file of files) {
  ...
  for (const match of source.matchAll(/id:\s*"([^"]+)"\s*,\s*slug:\s*"([^"]+)"\s*,\s*name:\s*"([^"]+)"[\s\S]{0,180}?aliases:\s*\[([^\]]*)\]/g)) {
    ...
    rows.push({ file, id: match[1], slug: match[2], name: match[3], key: normalize(match[3]), aliases });
  }
}
...
if (seedBlock) {
  for (const name of seedBlock[1].split(/\r?\n/).map((value) => value.trim()).filter(Boolean)) {
    rows.push({ file: "bulk-catalog.ts", id: `bulk:${slugify(name)}`, slug: slugify(name), name, key: normalize(name), aliases: [normalize(name)], bulk: true });
  }
}
