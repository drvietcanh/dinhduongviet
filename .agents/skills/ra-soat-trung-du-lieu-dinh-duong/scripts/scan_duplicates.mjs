...
const normalize = (value) => value.normalize("NFD").replace(/[\u0300-\u036f]/g, "").replace(/đ/g, "d").toLowerCase().replace(/[^a-z0-9]+/g, "").trim();
...
for (const file of files) {
  ...
  for (const match of source.matchAll(/id:\s*"([^"]+)"\s*,\s*slug:\s*"([^"]+)"\s*,\s*name:\s*"([^"]+)"/g)) {
    rows.push({ file, id: match[1], slug: match[2], name: match[3], key: normalize(match[3]) });
  }
}
...
if (seedBlock) {
  for (const name of seedBlock[1].split(/\r?\n/).map((value) => value.trim()).filter(Boolean)) {
    rows.push({ file: "bulk-catalog.ts", id: `bulk:${slugify(name)}`, slug: slugify(name), name, key: normalize(name), bulk: true });
  }
}
...
const groupBy = (field) => {
  ...
  return [...groups.entries()].filter(([, group]) => new Set(group.map((row) => row.id)).size > 1);
};
