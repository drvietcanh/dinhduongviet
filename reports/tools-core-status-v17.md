# Tools Core Status v17

Date: 2026-07-01

Branch: `tools-core-status-v17`

Base commit: `05789c1 test: finalize weekly menu page review`

Tag created:

- `local-lap-thuc-don-tuan-final-review-v1 -> 05789c1`

## Scope

- Promote `lap-thuc-don-tuan` to `stable_v1` after QA polish and final review.
- Keep total tool count at `38`.
- Update `/cong-cu/` with a light `Đã kiểm v1` badge for **Lập thực đơn tuần**.
...
Counts were recalculated directly from all 38 rows in the v16 status table. The v16 row-level counts were:
...
In v16, `lap-thuc-don-tuan` was classified as `needs_qa_polish`. This round moves only that row from `needs_qa_polish` to `stable_v1`; all other row-level statuses remain unchanged.

- `stable_v1`: 20 tools.
- `needs_spec`: 5 tools.
- `needs_implementation`: 0 tools in this pass; existing incomplete routes are classified as `stub_or_draft`.
- `needs_qa_polish`: 11 tools.
...
Updated the **Kế hoạch bữa ăn** card:

- Route remains `/cong-cu/ke-hoach-bua-an`.
- Badge: `Đã kiểm v1`.
- Description remains: `Lập khung bữa ăn tham khảo và nhắc các trường hợp cần hỏi bác sĩ hoặc chuyên gia.`
- The card remains in **Thực đơn, đi chợ, nhật ký**.

The card does not describe the route as:

- a treatment meal-plan tool.
- a disease-safe meal-plan tool.
- a disease target calculator.
- a personal sodium, potassium, phosphorus, carbohydrate, or protein target tool.
...
- Preview ran at `http://localhost:4379`.
- `/cong-cu/` has a **Kế hoạch bữa ăn** card with the `Đã kiểm v1` badge and safe description.
- `/cong-cu/ke-hoach-bua-an/` remains a real route that returns `200`, with no redirect or meta refresh.
- Browser plugin check passed in the in-app browser.
...
- Route output remains scoped to safe meal planning orientation + safety shell only.
- Form remained usable on mobile, output after submit did not overflow, the disease filter stayed safety-note only, user-facing BMR/TDEE remained absent, and the route did not show disease-target output, disease-safe conclusion, medication/insulin/diuretic guidance, or `innerHTML` rendering from user input.

## Recommended Next Work

- No `clinical_high_risk` P0 route remains in the current status table.
- `needs_qa_polish` is now `12`, so the next phase can move to the next prioritized tool in that queue.

## Conclusion

The project now has **19 stable v1 tools**. `ke-hoach-bua-an` is stable only as a safe meal planning orientation and safety shell for building a general reference meal frame and preparing users to seek individualized review when disease, medicines, pregnancy, childhood, older age, kidney disease, diabetes, hypertension, gout, heart failure, or treatment-oriented nutrition needs matter. It should not be treated as a therapeutic meal-plan tool, a disease target calculator, a disease-safe/fit meal classifier, a sodium/potassium/phosphorus/carbohydrate/protein disease-target tool, a medication/insulin/diuretic/meal-adjustment tool, an engine change, a formula change, or a nutrition data change.

`clinical_high_risk` is now **0**, and the P0 clinical high risk queue is complete in the current status table.

`needs_qa_polish` is now **12**, so the project is ready to move to the next tool in that queue.
