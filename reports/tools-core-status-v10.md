# Tools Core Status v10

Date: 2026-06-29

Branch: `tools-core-status-v10`

Base commit: `b043969 test: finalize pregnancy nutrition page review`

Tag created:

- `local-dinh-duong-thai-ky-final-review-v1 -> b043969`

## Scope

- Promote `dinh-duong-thai-ky` to `stable_v1` after safety patch, QA polish, and final review.
- Keep total tool count at `38`.
- Update `/cong-cu/` with a light `Đã kiểm v1` badge for **Dinh dưỡng thai kỳ**.
...
Counts were recalculated directly from all 38 rows in the v9 status table. The v9 row-level counts were:

- `stable_v1`: 12.
- `needs_spec`: 5.
- `needs_qa_polish`: 13.
- `stub_or_draft`: 2.
- `clinical_high_risk`: 6.

In v9, `dinh-duong-thai-ky` was classified as `clinical_high_risk`. This round moves only that row from `clinical_high_risk` to `stable_v1`; all other row-level statuses remain unchanged.
...
- `clinical_high_risk`: 5 tools.

Total: `38` tools.

## Thirteen Stable v1 Tools
...
Updated the **Dinh dưỡng thai kỳ** card:

- Route remains `/cong-cu/dinh-duong-thai-ky`.
- Badge: `Đã kiểm v1`.
- Description remains: `Nhắc các tình huống cần cá thể hóa khi đánh giá dinh dưỡng trong thai kỳ.`
...
- a treatment tool for anemia, gestational diabetes, or preeclampsia.
- a tool for self-supplementing micronutrients.
...
- `/cong-cu/` has a **Dinh dưỡng thai kỳ** card with the `Đã kiểm v1` badge and safe description.
- `/cong-cu/dinh-duong-thai-ky/` remains a real route that returns `200`, with no redirect or meta refresh.
- Route output remains scoped to educational safety shell only.
- Rendered route output does not show a form, BMI calculator, BMR-style formula, stage multiplier, numerical kcal/protein/micronutrient target, pregnancy weight-gain assessment, personal pregnancy meal plan, self-supplementation advice, or forbidden wording.
- Any favicon/static auxiliary 404 remains outside the route/app behavior and is not treated as a route failure.

## Recommended Next Tools

| Priority | Tool | Why next | Main risk | Recommended path |
|---:|---|---|---|---|
| 1 | `theo-doi-duong-huyet` | Next P0 from the existing clinical triage order after medication tools, child nutrition, and pregnancy nutrition. | Glucose thresholds/actions can be read as diabetes clinical advice. | Neutral logging/safety patch. |
| 2 | `theo-doi-suc-khoe` | Multi-metric clinical tracker remains high risk. | BP/glucose/A1c/LDL target lines can be read as treatment targets. | Neutral logging/safety patch. |
| 3 | `dia-an-lanh-manh` | Disease-specific plate route remains high risk. | Disease table includes kidney/gout/hypertension/diabetes-style advice. | Safety hub/orientation patch before status. |
| 4 | `nuoc-muoi-mon-an` | Salt/sodium route remains high risk. | Disease-specific sodium targets for hypertension, heart failure, and kidney disease. | Remove disease-specific targets or gate as clinical no-auto. |
| 5 | `chi-so-gi` | GI/GL route remains high risk for diabetes interpretation. | GI/GL outputs may be treated as diabetes treatment guidance. | Source-lock and safety wording before promotion. |

## Conclusion

The project now has **13 stable v1 tools**. `dinh-duong-thai-ky` is stable only as an educational safety shell for recognizing when pregnancy or lactation nutrition needs individualized review. It should not be treated as a pregnancy calorie or micronutrient calculator, pregnancy weight-gain assessment tool, meal plan generator, treatment tool, new engine, formula change, or nutrition data change.
