# Tools Core Status v8

Date: 2026-06-24

Branch: `tools-core-status-v8`

Base commit: `7e21796 test: finalize drug interaction page review`

Tag created:

- `local-tuong-tac-thuoc-final-review-v1 -> 7e21796`

## Scope

- Promote `tuong-tac-thuoc` to `stable_v1` after safety patch, QA polish, and final review.
- Keep total tool count at `38`.
- Update `/cong-cu/` with a light `Đã kiểm v1` badge for **Tương tác thuốc**.
- No engine logic changes.
- No formula changes.
- No nutrition data changes.
- No manual `dist` edits.
- No route logic changes to `muc-tieu-dinh-duong`, `muc-tieu-can-nang`, `so-sanh`, `tinh-macro`, `tinh-nang-luong`, `so-sanh-bua-an`, or `so-sanh-thuc-pham`.
...
| 37 | `tuong-tac-thuoc` | `stable_v1` | not needed, safety shell only | safety patch, QA polish, final review reports | yes, Browser plugin desktop/mobile 390px | yes, as safety shell only | Route `/cong-cu/tuong-tac-thuoc/`. User-facing name: **Tương tác thuốc**. Safety shell only for situations that need doctor/pharmacist review; no personal drug interaction lookup, drug search, user-facing database/list, severity/action/mechanism output, safe-combination conclusion, mild/moderate/severe tier, self-stop/dose/timing instruction, new engine, formula, or nutrition data change. |
| 38 | `tuong-tac-thuoc-thuc-pham` | `stable_v1` | not needed, safety shell only | safety patch, QA polish, final review reports | yes, Browser plugin desktop/mobile 390px | yes, as safety shell only | Route `/cong-cu/tuong-tac-thuoc-thuc-pham/`. User-facing name: **Tương tác thuốc - thực phẩm**. Safety shell only for situations that need doctor/pharmacist review; no personal interaction lookup, drug selector, hardcoded user-facing interaction table, safe-combination conclusion, severity tier, self-stop/dose/timing instruction, new engine, formula, or nutrition data change. |

## Summary Counts

Counts were recalculated directly from all 38 rows in the v7 status table. The v7 row-level counts were:

- `stable_v1`: 10.
- `needs_spec`: 5.
- `needs_qa_polish`: 13.
- `stub_or_draft`: 2.
- `clinical_high_risk`: 8.

In v7, `tuong-tac-thuoc` was classified as `clinical_high_risk`. This round moves only that row from `clinical_high_risk` to `stable_v1`; all other row-level statuses remain unchanged.
...
- Safety shell only.
- Reminds users about situations that should be discussed with a doctor or pharmacist.
- Supports preparation for a professional conversation.
- Keeps the route available as a safe page, not a personal drug interaction lookup.
- User-facing wording says the page does not replace doctor/pharmacist advice.

It is not:

- A personal drug interaction lookup.
- A drug interaction database.
- A medication advice engine.
- A two-drug safe-combination conclusion tool.
- A severity/action/mechanism tool.
- A calculator.
- A prescription or treatment tool.
- A substitute for a doctor or pharmacist.

It does not display or enable:

- A drug search box.
- A user-facing drug interaction database or list.
- `severity`, `action`, or `mechanism` output.
- A conclusion that two medicines can be used together safely.
- Mild/moderate/severe interaction tiers.
- Instructions to stop medicine.
- Instructions to change dose.
- Instructions to change medicine timing.
- A new engine.
- New formulas.
...
- `reports/tool-tuong-tac-thuoc-safety-patch-v1.md`
- `reports/tool-tuong-tac-thuoc-qa-polish-v1.md`
...
- H1 is `Tương tác thuốc`.
- Desktop/default viewport had no horizontal overflow.
- Mobile `390 x 844` Browser check had no horizontal overflow.
- No drug search was present.
- No user-facing database/list was present.
- No `severity`, `action`, or `mechanism` output was present.
- Five final groups returned only `Cần hỏi bác sĩ hoặc dược sĩ`:
  - dùng thuốc theo toa.
  - dùng nhiều thuốc cùng lúc.
  - thuốc chống đông hoặc thuốc tim mạch.
  - thuốc đái tháo đường hoặc đường huyết.
  - thuốc huyết áp hoặc thuốc lợi tiểu.
- Forbidden medication-action wording was not found in rendered route output.

## `tuong-tac-thuoc-thuc-pham` Stable Scope
...
- Reminds users about situations that should be discussed with a doctor or pharmacist.
- Supports preparation for a professional conversation.
- Keeps the route available as a safe page, not a personal lookup tool.
- User-facing wording says the page does not replace doctor/pharmacist advice.

It is not:

- A personal drug-food interaction lookup.
- A medication advice engine.
- A drug-specific safety conclusion tool.
- A calculator.
- A prescription or treatment tool.
- A substitute for a doctor or pharmacist.
...
- A drug-specific selector.
- A hardcoded interaction table shown to users.
- A conclusion that any drug-food pairing is safe.
- Mild/moderate/severe interaction tiers.
- Instructions to stop medicine.
- Instructions to change dose.
- Instructions to change medicine timing.
...
- `reports/clinical-high-risk-triage-v1.md`
- `reports/tool-tuong-tac-thuoc-thuc-pham-safety-patch-v1.md`
- `reports/tool-tuong-tac-thuoc-thuc-pham-qa-polish-v1.md`
...
- Desktop/default viewport had no horizontal overflow.
- Mobile `390 x 844` Browser check had no horizontal overflow.
- No drug-specific selector was present.
- No hardcoded user-facing interaction table was present.
- Five final groups returned only `Cần hỏi bác sĩ hoặc dược sĩ`:
  - thuốc theo toa.
  - thuốc chống đông hoặc thuốc tim mạch.
  - thuốc đái tháo đường hoặc đường huyết.
  - thuốc huyết áp, lợi tiểu hoặc điện giải.
...
Updated the **Tương tác thuốc** card:

- Route remains `/cong-cu/tuong-tac-thuoc`.
- Badge: `Đã kiểm v1`.
- Description remains: `Nhắc các tình huống cần hỏi bác sĩ hoặc dược sĩ khi dùng nhiều thuốc hoặc có bệnh nền.`
...
- `/cong-cu/` has a **Tương tác thuốc** card with the `Đã kiểm v1` badge and safe description.
- `/cong-cu/tuong-tac-thuoc/` remains a real route that returns `200`, with no redirect or meta refresh.
- Route output remains scoped to safety shell only.
- Rendered route output does not show a drug search, user-facing database/list, `severity`/`action`/`mechanism`, personal interaction conclusion, severity tier, self-stop/dose/timing instruction, or forbidden wording.

## Recommended Next Tools

| Priority | Tool | Why next | Main risk | Recommended path |
|---:|---|---|---|---|
| 1 | `nhu-cau-dinh-duong-tre-em` | Next P0 from the existing clinical triage order after the two medication tools. | Child numerical targets and growth-like interpretation. | Safety patch or source-lock before any promotion. |
| 2 | `dinh-duong-thai-ky` | Pregnancy nutrition route remains high risk. | Pregnancy weight gain, kcal, and micronutrient targets. | Safety patch or source-lock before any promotion. |
| 3 | `theo-doi-duong-huyet` | Diabetes-facing tracker remains high risk. | Thresholds/actions can be read as clinical advice. | Neutral logging/safety patch. |
| 4 | `theo-doi-suc-khoe` | Multi-metric clinical tracker remains high risk. | BP/glucose/A1c/LDL target lines can be read as treatment targets. | Neutral logging/safety patch. |
| 5 | `dia-an-lanh-manh` | Disease-specific plate route remains high risk. | Disease table includes kidney/gout/hypertension/diabetes-style advice. | Safety hub/orientation patch before status. |

## Conclusion

The project now has **11 stable v1 tools**. `tuong-tac-thuoc` is stable only as a safety shell for reminding users when to ask a doctor or pharmacist. It should not be treated as a personal drug interaction lookup, medication advice engine, safe-combination conclusion tool, prescription tool, treatment tool, severity/action/mechanism tool, new engine, formula change, or nutrition data change.
