# Tools Core Status v5

Date: 2026-06-22

Branch: `tools-core-status-v5`

Base commit: `e292187 test: finalize weight goal page review`

Tag created:

- `local-muc-tieu-can-nang-final-review-v1 -> e292187`

## Scope

- Promote `muc-tieu-can-nang` to `stable_v1` after audit/spec, safety patch, QA polish, and final review.
- Keep total tool count at `38`.
- Update `/cong-cu/` with a light `Đã kiểm v1` badge for **Mục tiêu cân nặng**.
- No engine logic changes.
- No formula changes.
- No nutrition data changes.
- No manual `dist` edits.
- No route logic changes to `muc-tieu-dinh-duong`, `muc-tieu-can-nang`, `tinh-macro`, or `tinh-nang-luong`.
...
Counts were recalculated directly from all 38 rows in the v4 status table. The v4 prose summary (`7/7/14/3/7`) did not match its own row-level classifications. Direct recount of the v4 table produced:
...
In v4, `muc-tieu-can-nang` was classified as `needs_spec`, not `needs_qa_polish`. This round moves only that row from `needs_spec` to `stable_v1`; all other row-level statuses remain unchanged.
...
Updated the **Mục tiêu cân nặng** card:

- Route remains `/cong-cu/muc-tieu-can-nang`.
- Badge: `Đã kiểm v1`.
- Description remains: `Định hướng mục tiêu cân nặng tham khảo với cờ cần cá thể hóa.`
...
- `/cong-cu/` has a **Mục tiêu cân nặng** card with the `Đã kiểm v1` badge and safe description.
- `/cong-cu/muc-tieu-can-nang/` remains a real route that returns `200`, with no redirect or meta refresh.
- User-facing route output remains scoped to safe orientation, mode, BMI screening, warnings, and allowed protein reference.
- Rendered output does not show BMR/TDEE, `7700 kcal/kg`, deficit/surplus, numerical kcal/macro targets, disease macro table, or forbidden wording.
...
| 2 | `so-sanh-bua-an` | Linked from stable meal hub. | Comparison totals and assumptions need QA. | QA/spec light pass, then polish. |
...
The project now has **8 stable v1 tools**. `muc-tieu-can-nang` is stable only as a safe weight goal orientation tool. It should not be treated as a weight-loss/weight-gain calculator, kcal/macro calculator, BMR/TDEE calculator, speed promise, treatment prescription, or disease-specific target tool.
