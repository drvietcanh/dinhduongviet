# Tools Core Status v7

Date: 2026-06-24

Branch: `tools-core-status-v7`

Base commit: `40c2af9 test: finalize drug food interaction page review`

Tag created:

- `local-tuong-tac-thuoc-thuc-pham-final-review-v1 -> 40c2af9`

## Scope

- Promote `tuong-tac-thuoc-thuc-pham` to `stable_v1` after safety patch, QA polish, and final review.
- Keep total tool count at `38`.
- Update `/cong-cu/` with a light `Đã kiểm v1` badge for **Tương tác thuốc - thực phẩm**.
...
Counts were recalculated directly from all 38 rows in the v6 status table. The v6 row-level counts were:
...
In v6, `tuong-tac-thuoc-thuc-pham` was classified as `clinical_high_risk`. This round moves only that row from `clinical_high_risk` to `stable_v1`; all other row-level statuses remain unchanged.
...
Updated the **Tương tác thuốc - thực phẩm** card:

- Route remains `/cong-cu/tuong-tac-thuoc-thuc-pham`.
- Badge: `Đã kiểm v1`.
- Description remains: `Nhắc các tình huống cần hỏi bác sĩ hoặc dược sĩ khi dùng thuốc cùng thực phẩm.`
...
No neighboring medication-facing route was promoted:

- `/cong-cu/tuong-tac-thuoc` remains `clinical_high_risk`.
...
- `/cong-cu/` has a **Tương tác thuốc - thực phẩm** card with the `Đã kiểm v1` badge and safe description.
- `/cong-cu/tuong-tac-thuoc-thuc-pham/` remains a real route that returns `200`, with no redirect or meta refresh.
- Route output remains scoped to safety shell only.
- Rendered route output does not show a drug-specific selector, hardcoded interaction table, personal interaction conclusion, severity tier, self-stop/dose/timing instruction, or forbidden wording.
- `/cong-cu/tuong-tac-thuoc` was not promoted by this status update.

## Recommended Next Tools

| Priority | Tool | Why next | Main risk | Recommended path |
|---:|---|---|---|---|
| 1 | `tuong-tac-thuoc` | Remaining medication-facing route with high-risk interaction content. | Drug-specific advice and severity/action wording. | Safety patch before deploy. |
| 2 | `nhu-cau-dinh-duong-tre-em` | Pediatric calculator remains high risk. | Child numerical targets and growth-like interpretation. | Safety patch or source-lock before any promotion. |
| 3 | `dinh-duong-thai-ky` | Pregnancy nutrition route remains high risk. | Pregnancy weight gain, kcal, and micronutrient targets. | Safety patch or source-lock before any promotion. |
| 4 | `theo-doi-duong-huyet` | Diabetes-facing tracker remains high risk. | Thresholds/actions can be read as clinical advice. | Neutral logging/safety patch. |
| 5 | `tra-cuu-thuc-pham-viet` | Core data lookup supports many workflows. | Missing values, units, and source-status confusion. | Dedicated QA polish after P0 clinical patches. |

## Conclusion

The project now has **10 stable v1 tools**. `tuong-tac-thuoc-thuc-pham` is stable only as a safety shell for reminding users when to ask a doctor or pharmacist. It should not be treated as a personal interaction lookup, medication advice engine, safe-combination conclusion tool, prescription tool, treatment tool, new engine, formula change, or nutrition data change.
