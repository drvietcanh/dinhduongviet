...
- QA polish commit: `d61ca13 test: add glycemic index page QA`.
- Tag created: `local-chi-so-gi-qa-polish-v1 -> d61ca13`.
- Branch: `tool-chi-so-gi-final-review-v1`.
- Chua deploy.
...
- Tao moi report: `reports/tool-chi-so-gi-final-review-v1.md`.

Khong sua route hoac `/cong-cu/` trong vong final review vi route da dat scope va wording an toan.

## 4. Route scope sau final review

Route `/cong-cu/chi-so-gi/` dat scope:
...
- Chay `npm run preview -- --port 4373`.
- Preview URL: `http://localhost:4373`.

Browser method:

- Dung in-app Browser tab hien co.
- Kiem DOM, console, desktop viewport va mobile viewport `390 x 844`.
- HTTP status duoc xac nhan rieng bang PowerShell `Invoke-WebRequest` voi `MaximumRedirection 0`.
- Screenshot desktop va mobile duoc chup trong browser session de xac nhan layout; khong commit screenshot vao repo.
...
- Page identity: title va H1 deu la `Chỉ số GI thực phẩm Việt`.
- DOM snapshot co noi dung route, khong blank.
- Framework error overlay: khong ghi nhan.
- Console app: sach, khong co warning/error.
- Desktop viewport `1280 x 720`: khong overflow ngang.
...
- Meta refresh: khong co.
- DOM snapshot co card `Chỉ số GI thực phẩm Việt`.
- Console app: sach, khong co warning/error.
...
- Card `chi-so-gi` chua gan badge `Đã kiểm v1`.
...
- Search/filter: khong con.
- Input user-facing: khong con.
- Table: khong con.
- Route-scoped script: khong con.
- `innerHTML`: khong con trong rendered HTML cua route.
- LocalStorage/chart/canvas: khong co.
...
Route user-facing con lai:

- Giai thich GI la khai niem tham khao ve toc do lam tang duong huyet cua thuc pham chua carbohydrate.
- Noi ro GI khong du de quyet dinh mot mon phu hop cho tung nguoi hay tung benh nen.
- Nhac khau phan, carbohydrate thuc te, cach che bien, mon an di kem, van dong va thuoc dang dung deu co the lam thay doi dap ung sau an.
- Nhac nguoi co dai thao duong, tien dai thao duong, thai ky, tre em, nguoi cao tuoi, benh than/gan/tim mach, dang dung insulin/thuoc ha duong huyet hoac can ca the hoa nen hoi bac si/chuyen gia dinh duong.
...
- `GI` la khai niem giao duc chinh cua route.
- `GL` xuat hien trong ten nhom/link stable `Tính GL từ bữa ăn`, khong phai bang lookup GI/GL cua route.
...
- `script`, `table`, `input` chi xuat hien trong source/grep khi la CSS/layout hoac cac card khac tren `/cong-cu/`, khong phai route-scoped UI cua `chi-so-gi`.
...
Worktree can sach sau commit final review.

## 13. Ket luan

`/cong-cu/chi-so-gi/` du dieu kien sang:

- `tools-core-status-v15`

De xuat stable scope:
