# Tools Core Status v22

Date: 2026-07-02

Branch: `tools-core-status-v22`

Base commit: `4f5e5f8 test: finalize meal comparison page review`

Tag created:

- `local-so-sanh-bua-an-final-review-v1 -> 4f5e5f8`

## Scope

- Promote `so-sanh-bua-an` to `stable_v1` after QA polish and final review.
- Keep total tool count at `38`.
- Update `/cong-cu/` with a light `Đã kiểm v1` badge for **So sánh bữa ăn**.
...
Counts were recalculated directly from all 38 rows in the v21 status table. The v21 row-level counts were:
...
In v21, `so-sanh-bua-an` was classified as `needs_qa_polish`. This round moves only that row from `needs_qa_polish` to `stable_v1`; all other row-level statuses remain unchanged.

- `stable_v1`: 25 tools.
- `needs_spec`: 5 tools.
- `needs_implementation`: 0 tools in this pass; existing incomplete routes are classified as `stub_or_draft`.
- `needs_qa_polish`: 6 tools.
...
The remaining `needs_qa_polish` tools after promoting `so-sanh-bua-an` are:

1. `bang-xep-hang`
2. `checklist-an-uong`
3. `danh-sach-di-cho`
4. `doi-don-vi`
5. `them-thuc-pham-dong-goi`
6. `tim-mon-tu-nguyen-lieu`

## Twenty-Five Stable v1 Tools
...
- Keeps neutral meal comparison flow for `2-4` meals.
- Keeps the compare table and summary only as neutral nutrition data display.
- Keeps user-facing data per `100g phần ăn được`.
- Shows energy in `kcal`.
- Shows protein, glucid, fat, fiber, and sugar in `g`.
- Shows sodium, potassium, calcium, iron, and vitamin C in `mg` when available.
- Keeps missing data neutral as `—` and does not guess missing values.
...
- A tool that picks a better meal.
- A tool that recommends which meal to choose.
- A good/bad meal classification tool for disease backgrounds.
- A safe/fit meal conclusion tool for people with disease.
- A personal nutrition target tool.
- A treatment, medication, insulin, or diuretic adjustment tool.

It does not display or enable:

- Winner/better/worse wording.
- `nên chọn` or recommendation wording.
- Disease filters.
- Disease-specific meal lists.
...
Neutral comparison wording retained:

- `cao hơn`
- `thấp hơn`

These labels remain only as neutral descriptions of data differences within the current comparison table and summary and not as personal health judgments or disease-treatment advice.

Data and unit notes:

- User-facing data is normalized to `kcal`, `g`, and `mg`.
- Data is displayed per `100g phần ăn được`.
- Missing data is rendered neutrally as `—`.
- Numeric zero remains distinct from missing data.
- Sodium is not renamed to salt without explicit conversion.
- Compare behavior does not convert missing data into zero-valued conclusions.
...
- QA polish: `6a2ed2c test: add meal comparison page QA`.
- Final review: `4f5e5f8 test: finalize meal comparison page review`.
...
- Meal selection flow worked for `2-4` meals.
- Compare table stayed within page width and remained usable on mobile.
- Empty state stayed clear and neutral when fewer than two meals were selected.
...
- The route stayed within neutral comparison scope and did not show winner wording, disease-safe wording, treatment advice, or personal nutrient targets.
...
- The first `npm run build` attempt hit the tool timeout window and passed on rerun with a longer timeout.
- An initial parallel `npm run qa:food-data` attempt started before the rerun build finished and failed on missing `dist/api-foods.json`; rerunning `qa:food-data` after build completion passed.
...
- `/cong-cu/so-sanh-bua-an/` remains `200`, no redirect, no meta refresh, console clean after fresh reload/session.
- `/cong-cu/` card for **So sánh bữa ăn** now carries badge `Đã kiểm v1`.
...
- No formula changes.
- No route logic changes outside previously completed review scope and status/report metadata.
- No `dist` edits.

## Worktree

- Before commit, `git status --short` should show only the intended status-round changes:
  - `?? reports/tools-core-status-v22.md`
- Worktree is expected to be clean after commit.

## Conclusion

- `so-sanh-bua-an` is ready to move from `needs_qa_polish` to `stable_v1`.
- Stable scope: `neutral meal comparison + data QA shell only`.
- Queue `needs_qa_polish` drops from `7` to `6`.
