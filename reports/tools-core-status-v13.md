# Tools Core Status v13

Date: 2026-06-29

Branch: `tools-core-status-v13`

Base commit: `6d444cf test: finalize healthy plate page review`

Tag created:

- `local-dia-an-lanh-manh-final-review-v1 -> 6d444cf`

## Scope

- Promote `dia-an-lanh-manh` to `stable_v1` after safety patch, QA polish, and final review.
- Keep total tool count at `38`.
- Update `/cong-cu/` with a light `Đã kiểm v1` badge for **Đĩa ăn lành mạnh**.
...
Counts were recalculated directly from all 38 rows in the v12 status table. The v12 row-level counts were:

- `stable_v1`: 15.
- `needs_spec`: 5.
- `needs_qa_polish`: 13.
- `stub_or_draft`: 2.
- `clinical_high_risk`: 3.

In v12, `dia-an-lanh-manh` was classified as `clinical_high_risk`. This round moves only that row from `clinical_high_risk` to `stable_v1`; all other row-level statuses remain unchanged.
...
- `clinical_high_risk`: 2 tools.

Total: `38` tools.

## Sixteen Stable v1 Tools
...
Updated the **Đĩa ăn lành mạnh** card:

- Route remains `/cong-cu/dia-an-lanh-manh`.
- Badge: `Đã kiểm v1`.
- Description remains: `Định hướng đĩa ăn chung và nhắc các tình huống cần cá thể hóa khi có bệnh nền.`
- The card remains in **Thực đơn, đi chợ, nhật ký**.

The card does not describe the route as:

- a calculator.
- a disease-specific plate ratio tool.
- a personal meal-plan or menu-prescription tool.
- a sodium, potassium, phosphorus, water, protein, kcal, or macro target tool.
- a disease treatment tool.
- a tool for concluding that a dish or plate is safe for a disease background.
...
- `/cong-cu/` has a **Đĩa ăn lành mạnh** card with the `Đã kiểm v1` badge and safe description.
- `/cong-cu/dia-an-lanh-manh/` remains a real route that returns `200`, with no redirect or meta refresh.
- Route output remains scoped to educational orientation + safety shell only.
- Rendered route output does not show disease selector, form/calculator, user-facing disease table, disease-personalized SVG plate, disease-specific plate ratio, personal target numbers, avoid/replace instructions by disease, safe-for-disease conclusions, or forbidden wording.
- Stable related links recorded for the route remain:
  - `/cong-cu/khau-phan-viet-clinical`
  - `/cong-cu/muc-tieu-dinh-duong`
  - `/cong-cu/nuoc-uong`
...
|---:|---|---|---|---|
| 1 | `nuoc-muoi-mon-an` | Next remaining P0 after healthy plate. | Disease-specific sodium targets for hypertension, heart failure, and kidney disease. | Remove disease-specific targets or gate as clinical no-auto. |
| 2 | `chi-so-gi` | GI/GL route remains high risk for diabetes interpretation. | GI/GL outputs may be treated as diabetes treatment guidance. | Source-lock and safety wording before promotion. |

## Conclusion

The project now has **16 stable v1 tools**. `dia-an-lanh-manh` is stable only as an educational orientation and safety shell for general healthy plate guidance and preparing users to seek individualized review when disease background matters. It should not be treated as a disease-specific plate calculator, personal menu-prescription tool, sodium/potassium/phosphorus/water/protein/kcal/macro target tool, safe-for-disease conclusion tool, treatment tool, new engine, formula change, or nutrition data change.
