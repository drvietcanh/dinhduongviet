# Tools Core Status v2

Date: 2026-06-11

Branch: `tools-core-status-v2`

Base commit: `23ae51f test: add Vietnamese meal assessment QA`

Tag created:

- `local-khau-phan-assessment-v1 -> 23ae51f`

## Scope

- Promote `khau-phan-viet-clinical` to `stable_v1` after spec, implementation, browser QA, and test coverage.
- Keep total tool count at 38.
- Add the light `Đã kiểm v1` badge for the user-facing **Đánh giá khẩu phần Việt** card on `/cong-cu/`.
- No formula changes.
...
| 13 | `khau-phan-viet-clinical` | `stable_v1` | yes, `npm run test:meal-assessment` | spec, implement, QA reports | yes, local browser QA | yes, within scope | User-facing name: **Đánh giá khẩu phần Việt**. Route remains `/cong-cu/khau-phan-viet-clinical/`. Stable only as education/meal assessment, not disease diet prescription. |
...
|---:|---|---|---|---|
| 1 | `danh-gia-bua-an` | Current placeholder overlaps with the new meal assessment and GL/carb workflows. | Duplicate concepts could confuse users. | Spec first; decide whether to merge, redirect, or rebuild as a lighter scoring tool. |
| 2 | `muc-tieu-dinh-duong` | Stub with broad target-setting scope. | Unsafe macro/calorie prescriptions if implemented loosely. | Spec first; define healthy vs clinical no-auto boundaries. |
| 3 | `so-sanh` | Stub and likely should be a hub for comparison tools. | Duplicate route with unclear promise. | Decide hub vs redirect; then implement lightly. |
| 4 | `tinh-macro` | High user demand and currently draft. | Macro targets can be mistaken for treatment targets. | Source-lock/spec first, then engine with clinical gate. |
| 5 | `tinh-nang-luong` | Stub but important for calorie planning. | Overlap with `muc-tieu-can-nang` and TDEE tools. | Spec first; consolidate naming and formulas. |
| 6 | `theo-doi-duong-huyet` | High clinical value. | Diabetes thresholds/actions vary by medication and patient. | Source-lock first, with no medication adjustment advice. |
| 7 | `nuoc-muoi-mon-an` | Sodium guidance is useful and clinically sensitive. | Hypertension, CKD, HF sodium advice needs personalization. | Source/spec first, then data and wording QA. |
| 8 | `dinh-duong-thai-ky` | Important audience and frequent need. | Pregnancy advice requires trimester and risk-specific sources. | Source-lock/spec first. |
| 9 | `chi-so-gi` | GI lookup supports carb/GL tools. | Diabetes interpretation can become overly deterministic. | Curate GI source table and wording. |
| 10 | `tuong-tac-thuoc-thuc-pham` | Medication safety is valuable. | Outdated or incomplete interaction guidance can be harmful. | Source/severity taxonomy first. |

## `/cong-cu/` UI Update

Added `Đã kiểm v1` badge for:

- `khau-phan-viet-clinical`

The card now uses the user-facing title:

- **Đánh giá khẩu phần Việt**

The route remains unchanged:

- `/cong-cu/khau-phan-viet-clinical`

The page still uses light badges only for stable tools and keeps draft tools separated, so unfinished tools are not presented as equal to checked v1 tools.
...
The project now has **5 stable v1 tools**. `khau-phan-viet-clinical` is stable only under its safer user-facing concept, **Đánh giá khẩu phần Việt**: educational assessment, not disease-specific diet prescription.

The next best move is to clean up the remaining overlapping draft tools, especially `danh-gia-bua-an`, `muc-tieu-dinh-duong`, `so-sanh`, `tinh-macro`, and `tinh-nang-luong`, before taking on another high-risk clinical tool.
