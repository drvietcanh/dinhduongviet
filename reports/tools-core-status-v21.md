# Tools Core Status v21

Date: 2026-07-02

Branch: `tools-core-status-v21`

Base commit: `ea4793b test: finalize food comparison page review`

Tag created:

- `local-so-sanh-thuc-pham-final-review-v1 -> ea4793b`

## Scope

- Promote `so-sanh-thuc-pham` to `stable_v1` after QA polish and final review.
- Keep total tool count at `38`.
- Update `/cong-cu/` with a light `Đã kiểm v1` badge for **So sánh thực phẩm**.
...
Counts were recalculated directly from all 38 rows in the v20 status table. The v20 row-level counts were:
...
In v20, `so-sanh-thuc-pham` was classified as `needs_qa_polish`. This round moves only that row from `needs_qa_polish` to `stable_v1`; all other row-level statuses remain unchanged.

- `stable_v1`: 24 tools.
- `needs_spec`: 5 tools.
- `needs_implementation`: 0 tools in this pass; existing incomplete routes are classified as `stub_or_draft`.
- `needs_qa_polish`: 7 tools.
...
## `so-sanh-thuc-pham` Stable Scope
...
- Keeps accent-insensitive Vietnamese food search and neutral comparison flow for `2-4` foods.
- Keeps the compare table only as neutral nutrient data display.
- Keeps user-facing data per `100g phần ăn được`.
- Shows energy in `kcal`.
- Shows macros in `g`.
- Shows sodium, minerals, phosphorus, cholesterol in `mg` when available.
- Shows vitamin A and folate in `µg` when available.
...
- A tool that picks a better food.
- A tool that recommends which food to choose.
...
These labels remain only as neutral descriptions of data differences within the current comparison table and not as personal health judgments or disease-treatment advice.

Data and unit notes:

- User-facing data is normalized to `kcal`, `g`, `mg`, and `µg`.
...
- Search/compare behavior does not convert missing data into zero-valued conclusions.
...
- This was not changed in the v21 status round.
...
- QA polish: `84cee94 test: add food comparison page QA`.
- Final review: `ea4793b test: finalize food comparison page review`.
...
- Search passed both accented and unaccented queries such as `nước mắm` and `nuoc mam`.
...
- Empty state stayed clear and neutral when fewer than two foods were selected or no result was found.
...
- `/cong-cu/so-sanh-thuc-pham/` remains `200`, no redirect, no meta refresh, console clean after fresh reload/session.
- `/cong-cu/` card for **So sánh thực phẩm** now carries badge `Đã kiểm v1`.
...
- `so-sanh-thuc-pham` is ready to move from `needs_qa_polish` to `stable_v1`.
- Stable scope: `neutral food comparison + data QA shell only`.
- Queue `needs_qa_polish` drops from `8` to `7`.
