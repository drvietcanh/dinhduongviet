# Tools Core Status v15

Date: 2026-06-30

Branch: `tools-core-status-v15`

Base commit: `a801500 test: finalize glycemic index page review`

Tag created:

- `local-chi-so-gi-final-review-v1 -> a801500`

## Scope

- Promote `chi-so-gi` to `stable_v1` after safety patch, QA polish, and final review.
- Keep total tool count at `38`.
- Update `/cong-cu/` with a light `Đã kiểm v1` badge for **Chỉ số GI thực phẩm Việt**.
...
Counts were recalculated directly from all 38 rows in the v14 status table. The v14 row-level counts were:

- `stable_v1`: 17.
- `needs_spec`: 5.
- `needs_qa_polish`: 13.
- `stub_or_draft`: 2.
- `clinical_high_risk`: 1.

In v14, `chi-so-gi` was classified as `clinical_high_risk`. This round moves only that row from `clinical_high_risk` to `stable_v1`; all other row-level statuses remain unchanged.
...
Updated the **Chỉ số GI thực phẩm Việt** card:

- Route remains `/cong-cu/chi-so-gi`.
- Badge: `Đã kiểm v1`.
- Description remains: `Tìm hiểu ý nghĩa chỉ số GI ở mức tham khảo và các tình huống cần hỏi bác sĩ hoặc chuyên gia.`
- The card remains in **Carb, đường huyết, GI/GL**.

The card does not describe the route as:

- a calculator.
- a diabetes meal-choice tool.
- a personal GI/GL lookup tool.
- a disease-safe food classifier.
- a blood glucose, HbA1c, carbohydrate, or GI target tool.
- a medication, insulin, meal, or treatment-adjustment tool.
...
- Preview ran at `http://localhost:4374`.
- `/cong-cu/` has a **Chỉ số GI thực phẩm Việt** card with the `Đã kiểm v1` badge and safe description.
- `/cong-cu/chi-so-gi/` remains a real route that returns `200`, with no redirect or meta refresh.
- Browser plugin check passed on the existing in-app tab.
...
- Route output remains scoped to educational GI orientation + safety shell only.
- Rendered route output does not show input, search, filter, table, route-scoped script, `innerHTML`, GI/GL table, low/medium/high health classification, personal target, disease-safe conclusion, medication/insulin/meal adjustment advice, or forbidden wording.

## Recommended Next Work

- No `clinical_high_risk` P0 route remains in the current status table.
- The next phase can move to whichever `needs_qa_polish` or `needs_spec` queue the project wants to prioritize next.

## Conclusion

The project now has **18 stable v1 tools**. `chi-so-gi` is stable only as an educational GI orientation and safety shell for understanding GI as a general concept and preparing users to seek individualized review when diabetes, prediabetes, pregnancy/lactation, childhood, older age, kidney disease, hypoglycemia history, insulin use, or glucose-lowering medicines matter. It should not be treated as a food-choice tool for diabetes, a personal GI/GL calculator, a disease-safe food classifier, a blood glucose/HbA1c/carb/GI target tool, a medication/insulin/meal adjustment tool, a new engine, a formula change, or a nutrition data change.
