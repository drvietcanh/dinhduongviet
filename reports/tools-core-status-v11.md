# Tools Core Status v11

Date: 2026-06-29

Branch: `tools-core-status-v11`

Base commit: `ed5ad48 test: finalize blood glucose tracking page review`

Tag created:

- `local-theo-doi-duong-huyet-final-review-v1 -> ed5ad48`

## Scope

- Promote `theo-doi-duong-huyet` to `stable_v1` after safety patch, QA polish, and final review.
- Keep total tool count at `38`.
- Update `/cong-cu/` with a light `Đã kiểm v1` badge for **Theo dõi đường huyết**.
...
Counts were recalculated directly from all 38 rows in the v10 status table. The v10 row-level counts were:

- `stable_v1`: 13.
- `needs_spec`: 5.
- `needs_qa_polish`: 13.
- `stub_or_draft`: 2.
- `clinical_high_risk`: 5.

In v10, `theo-doi-duong-huyet` was classified as `clinical_high_risk`. This round moves only that row from `clinical_high_risk` to `stable_v1`; all other row-level statuses remain unchanged.
...
- `clinical_high_risk`: 4 tools.

Total: `38` tools.

## Fourteen Stable v1 Tools
...
Updated the **Theo dõi đường huyết** card:

- Route remains `/cong-cu/theo-doi-duong-huyet`.
- Badge: `Đã kiểm v1`.
- Description remains: `Ghi nhật ký đường huyết và nhắc các tình huống cần hỏi bác sĩ hoặc chuyên gia.`
- The card remains in **Carb, đường huyết, GI/GL**.
...
- a glucose-control assessment tool.
- an HbA1c target tool.
- a good/high/low classification tool.
...
- `/cong-cu/` has a **Theo dõi đường huyết** card with the `Đã kiểm v1` badge and safe description.
- `/cong-cu/theo-doi-duong-huyet/` remains a real route that returns `200`, with no redirect or meta refresh.
- Route output remains scoped to neutral tracking log + safety shell only.
- Route keeps localStorage only as a local diary store through `ddv_glucose_log_v1`.
- Rendered route output does not show glucose targets, HbA1c targets, reference lines, good/high/low labels, hypo/hyper counts, reached/missed target conclusions, medication/insulin/meal adjustment instructions, or forbidden wording.

## Recommended Next Tools

| Priority | Tool | Why next | Main risk | Recommended path |
|---:|---|---|---|---|
| 1 | `theo-doi-suc-khoe` | Next P0 from the existing clinical triage order after glucose tracking. | BP/glucose/A1c/LDL target lines can be read as treatment targets. | Neutral logging/safety patch. |
| 2 | `dia-an-lanh-manh` | Disease-specific plate route remains high risk. | Disease table includes kidney/gout/hypertension/diabetes-style advice. | Safety hub/orientation patch before status. |
| 3 | `nuoc-muoi-mon-an` | Salt/sodium route remains high risk. | Disease-specific sodium targets for hypertension, heart failure, and kidney disease. | Remove disease-specific targets or gate as clinical no-auto. |
| 4 | `chi-so-gi` | GI/GL route remains high risk for diabetes interpretation. | GI/GL outputs may be treated as diabetes treatment guidance. | Source-lock and safety wording before promotion. |

## Conclusion

The project now has **14 stable v1 tools**. `theo-doi-duong-huyet` is stable only as a neutral tracking log and safety shell for recording user-entered diary data and preparing for clinician review. It should not be treated as a glucose-control assessment tool, HbA1c target tool, medication-adjustment tool, treatment tool, new engine, formula change, or nutrition data change.
